...
class Youtube extends Component{
    ...
    clicked(){
        fetch(finalURL)
            ...
            .then((responseJson) => {
                ...
                const resultyt  = responseJson.items.map(obj => "https://www.youtube.com/embed/"+obj.contentDetails.videoId);
    ...
    render(){
        ...
        return(
            <div>
                <div className= "btn-1">
                    <button className="btn btn-primary" onClick={this.clicked}>Get youtube videos</button>
                </div>
                {
                    this.state.resultyt.map((link, i)=>{
                        var frame = <div key={i} className="youtube"><iframe width="560" height="315" src={link} frameBorder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe></div>
                        return frame;
                    })
                }
